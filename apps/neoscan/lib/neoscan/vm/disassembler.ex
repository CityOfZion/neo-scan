defmodule Disassembler do
    ...
    |> Enum.reduce("", fn({code, index}, acc) ->
      cond do
        Map.has_key?(@opcodes_list, code) and String.length(code) == 2 ->
          ...
          acc <> to_string(index) <> ": " <> @opcodes_list[code] <> newline
        true ->
  ...
  # TODO figure out how to parse negative signed integers on JMP opcodes!
    ...
    parsed_list =
      for line <- list do
        case String.contains?(line, "SYSCALL") do
         false -> line
         true ->
           newline = Regex.replace(~r/PUSHBYTES[0-9][0-9] 0x/, line, "")
           newline_list = String.split(newline, " ")
           syscall_fn = case Base.decode16(Enum.at(newline_list, 2), case: :lower) do
             {:ok, syscall_arg} -> syscall_arg
             :error -> "error parsing syscall fn arg"
           end
           Enum.at(newline_list, 0) <> Enum.at(newline_list, 1) <> " " <> syscall_fn
        end
      end
    Enum.join(parsed_list, "\n")
  end
end
