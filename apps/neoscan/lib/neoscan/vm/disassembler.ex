defmodule Neoscan.Vm.Disassembler do
  ...
  @opcodes_list %{
    ...
    "PUSH0" => "PUSHF",
    "01" =>
      "PUSHBYTES1", # 0x01-0x4B The next opcode bytes is data to be pushed onto the stack
    "4b" => "PUSHBYTES75",
    "4c" =>
    ...
    "6a" => "DUPFROMALTSTACK",
    "6b" =>
      "TOALTSTACK", # Puts the input onto the top of the alt stack. Removes it from the main stack.
    "6c" =>
      "FROMALTSTACK", # Puts the input onto the top of the main stack. Removes it from the alt stack.
    "6d" => "XDROP",
    "72" => "XSWAP",
    "73" => "XTUCK",
    ...
    |> Enum.reduce(
         ...
         fn (current, {remaining, acc}) ->
           ...
           with true <- remaining_head == current,
             ...
             {opcodes_to_be_joined, new_remaining} = Enum.split(
               remaining,
               joins
             )
