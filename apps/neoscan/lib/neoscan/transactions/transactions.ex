defmodule Neoscan.Transactions do
  ...
  def count_transactions(type_list \\ @type_list) do
    query = from(t in Transaction, where: t.type in ^type_list)
  ...
  def count_transactions_for_asset(asset_hash) do
    query = from(t in Transaction, where: t.asset_moved == ^asset_hash)
  ...
  def create_transaction(
    ...
    address_list =
      Task.async(fn ->
        Addresses.get_transaction_addresses(new_vin, vouts, time, attrs["asset"])
  ...
  def update_transaction_state(%{:type => type} = transaction, _vouts)
