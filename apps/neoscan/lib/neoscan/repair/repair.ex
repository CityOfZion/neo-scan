defmodule Neoscan.Repair do
  ...
  def repair_missing(result, root) do
    missing =
      Enum.map(root, fn %{"txid" => txid} -> String.slice(to_string(txid), -64..-1) end) --
        Enum.map(result, fn %{:txid => txid} -> txid end)
  ...
  def get_missing(missing, root) do
    tuples =
      Enum.map(missing, fn txid -> get_transaction(txid) end)
      |> check_missing()
  ...
  def check_missing(transactions) do
    Enum.map(transactions, fn {:ok, %{"blockhash" => block_hash} = transaction} ->
  ...
  def check_if_vouts_exists(db_transaction, transaction) do
    ...
    missing =
      if db_vouts == [] do
        ...
        Enum.map(transaction["vout"], fn %{"n" => n} -> n end) --
          Enum.map(db_vouts, fn %{:n => n} -> n end)
      end
  ...
  def add_missing_transactions(:ok, tuples) do
    case Enum.any?(tuples, fn {key, _value} -> key == :transaction_missing end) do
      true ->
        ...
        |> Map.to_list()
        |> Enum.map(fn {block, transaction_tuples} ->
          {block, filter_tuples(transaction_tuples)}
        end)
        |> Enum.map(fn {block, transaction_list} ->
          Transactions.create_transactions(block, transaction_list)
        end)
        |> Enum.uniq()
