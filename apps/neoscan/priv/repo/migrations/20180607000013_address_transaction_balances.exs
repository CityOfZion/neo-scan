    ...
    execute """
      CREATE OR REPLACE FUNCTION flush_address_transaction_balances_queue()
        ...
        BEGIN
            ...
            WITH
            aggregated_queue AS (
                SELECT address_hash, transaction_hash, transaction_id, asset_hash, SUM(value) as value, MIN(block_time) as block_time, MIN(inserted_at) as inserted_at, MAX(updated_at) as updated_at
                FROM address_transaction_balances_queue
                GROUP BY address_hash, transaction_hash, transaction_id, asset_hash
            ),
            perform_updates AS (
                INSERT INTO address_transaction_balances
                SELECT address_hash, transaction_hash, transaction_id, asset_hash, value, block_time, inserted_at, updated_at
            ...
            perform_prune AS (
                DELETE FROM address_transaction_balances_queue
                RETURNING 1
            )
            SELECT
